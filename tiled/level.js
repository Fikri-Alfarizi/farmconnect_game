class Level extends AbstractTiledMap {
    ...
    draw(ctx) {
        ...
        // Draw Target Tile Highlight for Gamepad
        if (typeof Controller !== 'undefined' && Controller.isGamepadActive && Level.PLAYER.notDisablePlayerController()) {
            const [tx, ty] = Level.PLAYER.getTargetBlock();
            ctx.fillStyle = 'rgba(255, 255, 255, 0.2)';
            ctx.fillRect(tx * this.getTileSize() + this.getPixelX(), ty * this.getTileSize() + this.getPixelY(), this.getTileSize(), this.getTileSize());
            ctx.strokeStyle = 'white';
            ctx.lineWidth = 1;
            ctx.strokeRect(tx * this.getTileSize() + this.getPixelX(), ty * this.getTileSize() + this.getPixelY(), this.getTileSize(), this.getTileSize());
        }
        ...
        const entitiesThatCollideWithPlayer = this.getEntitiesThatCollideWith(Level.PLAYER)
        this.interactionAvailable = false; // Reset interaction flag
        if (entitiesThatCollideWithPlayer.length > 0) {
            // Prioritize Chests over NPCs for now if both collide, or just take first
            const interactable = entitiesThatCollideWithPlayer.find(e => e instanceof Chest || e instanceof Npc);
            if (interactable) {
                const _fontSize = Level.PLAYER.getMapReference().getTileSize() / 2
                const label = interactable instanceof Chest ? "Buka" : "Interaksi";
                this.interactionAvailable = true;

                if (Level.PLAYER.notDisablePlayerController()) {
                    MessageButton.draw(
                        GAME_ENGINE.ctx, label, _fontSize,
                        Level.PLAYER.getMapReference().getPixelX() + Level.PLAYER.getPixelRight() - _fontSize / 3, Level.PLAYER.getMapReference().getPixelY() + Level.PLAYER.getPixelY() + _fontSize
                    );

                    // Mouse click, keyboard E, OR gamepad A button
                    const triggerInteract = (!Controller.mouse_prev.leftClick && Controller.mouse.leftClick)
                        || (Controller.keys["KeyE"] && !Controller.keys_prev["KeyE"])
                        || (typeof GamepadController !== 'undefined' && GamepadController.isJustPressed('btnA'));

                    if (triggerInteract) {
                        if (interactable instanceof Chest) GAME_ENGINE.getPlayerUi().openChest(interactable);
                        else interactable.interact();

                        Controller.mouse.leftClick = false;
                        Controller.keys["KeyE"] = false;
                        Controller.keys["KeyQ"] = false; // Consume water press too
                    }
                }
            }
        }
